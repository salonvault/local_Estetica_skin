...
const imageCards = [
  { name: "Estetica care", service: "Acne Scar Treatment", image: "/assets/new/review1.jpeg" },
  { name: "Estetica care", service: "HydraFacial", image: "/assets/new/review3.jpeg" },
  { name: "Estetica care", service: "Melasma Care", image: "/assets/new/review_4.jpeg" },
  { name: "Estetica care", service: "Laser Hair Removal", image: "/assets/new/review_2.jpeg" },
];
...
function ImageReviewCard({ name, service, image, tall = false }: { name: string; service: string; image: string; tall?: boolean }) {
  return (
    <article className={`relative overflow-hidden rounded-[8px] bg-[var(--color-primary-3)] shadow-[0_14px_34px_rgba(24,24,27,0.1)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_52px_rgba(24,24,27,0.18)] ${tall ? "min-h-[410px]" : "min-h-[280px]"}`}>
      <Image src={image} alt={`${service} treatment at Estetica`} fill sizes="(min-width: 1024px) 25vw, 100vw" className="object-cover" />
      ...
      <div className="absolute bottom-5 left-5 right-5 text-white">
        ...
        <p className="mt-1 text-[9px] font-bold uppercase tracking-[0.08em] text-white/74">{service}</p>
      </div>
    </article>
  );
}
    ...
    <section id="testimonials" className="bg-[var(--warm-cream)] py-14 text-[var(--color-foreground)] lg:py-20">
      <div className="container">
        <div className="mx-auto mb-9 max-w-[760px] text-center">
          <p className="eyebrow">4.8 rating · 740 public reviews</p>
          <div className="mt-3">
            <Reveal>
